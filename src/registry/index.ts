...
import { createToolRegistry as subBassTools } from "../modules/subbass/tools.js";
import { createToolRegistry as organTools } from "../modules/organ/tools.js";
import { createToolRegistry as vocalChopTools } from "../modules/vocalchop/tools.js";
import { createToolRegistry as instrumentRenderTools } from "../modules/instrumentrender/tools.js";
import { createToolRegistry as brassTools } from "../modules/brass/tools.js";
import { createToolRegistry as wobbleTools } from "../modules/wobble/tools.js";
import { createToolRegistry as choirTools } from "../modules/choir/tools.js";
import { createToolRegistry as subDropTools } from "../modules/subdrop/tools.js";
import { createToolRegistry as pluckBassTools } from "../modules/pluckbass/tools.js";
import { createToolRegistry as sawLeadTools } from "../modules/sawlead/tools.js";
import { createToolRegistry as reeseTools } from "../modules/reese/tools.js";
import { createToolRegistry as marimbaTools } from "../modules/marimba/tools.js";
import { createToolRegistry as glitchTools } from "../modules/glitch/tools.js";
import { createToolRegistry as tapeHissTools } from "../modules/tapehiss/tools.js";
import { createToolRegistry as trumpetTools } from "../modules/trumpet/tools.js";
import { createToolRegistry as ePianoTools } from "../modules/epiano/tools.js";
import { createToolRegistry as musicBoxTools } from "../modules/musicbox/tools.js";
import { createToolRegistry as harpTools } from "../modules/harp/tools.js";
import { createToolRegistry as whistleTools } from "../modules/whistle/tools.js";
import { createToolRegistry as subWobbleTools } from "../modules/subwobble/tools.js";
import { createToolRegistry as vocoderTools } from "../modules/vocoder/tools.js";
import { createToolRegistry as noiseFxTools } from "../modules/noisefx/tools.js";
import { createToolRegistry as cymbalTools } from "../modules/cymbal/tools.js";
import { createToolRegistry as guitarTools } from "../modules/guitar/tools.js";
import { createToolRegistry as sitarTools } from "../modules/sitar/tools.js";
import { createToolRegistry as steelDrumTools } from "../modules/steeldrum/tools.js";
import { createToolRegistry as accordionTools } from "../modules/accordion/tools.js";
import { createToolRegistry as thereminTools } from "../modules/theremin/tools.js";
import { createToolRegistry as hiHat808Tools } from "../modules/hihat808/tools.js";
import { createToolRegistry as stabHitTools } from "../modules/stabhit/tools.js";
...
export function createMasterRegistry(): MasterRegistry {
  ...
  m.addModule({ id:"subbass", label:"Sub Bass", icon:"🔈", description:"Synthesizes a sustained tonal sub-bass in-host (sine + upper harmonics + drive) and imports as a new clip.", registry: subBassTools() });
  m.addModule({ id:"organ", label:"Organ", icon:"🎛️", description:"Additive drawbar organ in-host (Hammond ratios per chord note, percussion, vibrato) imported as a new clip.", registry: organTools() });
  m.addModule({ id:"vocalchop", label:"Vocal Chop", icon:"🗣️", description:"Synthesizes a chopped vocal texture in-host (formant-filtered saw gated into rhythmic steps) and imports as a new clip.", registry: vocalChopTools() });
  m.addModule({ id:"instrumentrender", label:"Instrument Render", icon:"🎚️", description:"Renders any MIDI clip (chords, melody, bass, drums, arp, pattern…) through any in-host engine into a new audio clip. The bridge between our engines and every MIDI generator.", registry: instrumentRenderTools() });
  m.addModule({ id:"brass", label:"Brass", icon:"🎺", description:"Synthesizes a synth-brass section in-host (stacked saws through a resonant filter envelope with vibrato) and imports as a new clip.", registry: brassTools() });
  m.addModule({ id:"wobble", label:"Wobble Bass", icon:"🌊", description:"Synthesizes a tempo-synced wobble bass in-host (saw stack + LFO-modulated low-pass + sub) and imports as a new clip.", registry: wobbleTools() });
  m.addModule({ id:"choir", label:"Choir", icon:"👥", description:"Synthesizes a choir / vocal pad in-host (detuned saws through vocal formants, slow attack, chorus) and imports as a new clip.", registry: choirTools() });
  m.addModule({ id:"subdrop", label:"Sub Drop", icon:"📉", description:"Synthesizes a sub-drop / downlifter in-host (sine glide-down with click and sub layer) for transitions.", registry: subDropTools() });
  m.addModule({ id:"pluckbass", label:"Pluck Bass", icon:"🎸", description:"Synthesizes a finger-pluck / bass-guitar tone (Karplus-Strong at bass register + sub + drive).", registry: pluckBassTools() });
  m.addModule({ id:"sawlead", label:"Saw Lead", icon:"⚡", description:"Synthesizes a detuned supersaw lead through a resonant filter — the classic trance / EDM lead.", registry: sawLeadTools() });
  m.addModule({ id:"reese", label:"Reese Bass", icon:"🐅", description:"Synthesizes a snarling Reese bass (two saws detuned and beating, slow LFO on cutoff, saturation).", registry: reeseTools() });
  m.addModule({ id:"marimba", label:"Marimba", icon:"🪵", description:"Synthesizes a wooden mallet / xylo tone (sine + inharmonic odd partials, fast decay, soft click).", registry: marimbaTools() });
  m.addModule({ id:"glitch", label:"Glitch FX", icon:"⚡", description:"Seed-driven mosaic of micro-slices each randomly processed (reverse, bitcrush, stutter) — IDM-style FX clips.", registry: glitchTools() });
  m.addModule({ id:"tapehiss", label:"Tape Hiss", icon:"📼", description:"Pink-ish band-limited noise bed with wow/flutter and crackle pops — a vinyl/tape lo-fi texture layer.", registry: tapeHissTools() });
  m.addModule({ id:"trumpet", label:"Trumpet", icon:"🎺", description:"Single-voice synth trumpet / solo brass (saw through formant band-pass with vibrato).", registry: trumpetTools() });
  m.addModule({ id:"epiano", label:"E-Piano", icon:"🎹", description:"Rhodes / DX7-style electric piano (2-op FM with low index, soft hammer, long sustain).", registry: ePianoTools() });
  m.addModule({ id:"musicbox", label:"Music Box", icon:"🎼", description:"Music-box tine (sine + metallic odd inharmonic partials, fast decay).", registry: musicBoxTools() });
  m.addModule({ id:"harp", label:"Plucked Harp", icon:"🪕", description:"Long-sustain Karplus-Strong strummed across a chord — harp / gliss-friendly plucked strings.", registry: harpTools() });
  m.addModule({ id:"whistle", label:"Whistle", icon:"😗", description:"Near-pure synth whistle (sine + light vibrato + breath noise at the attack).", registry: whistleTools() });
  m.addModule({ id:"subwobble", label:"Sub Wobble", icon:"🌀", description:"Sub-bass wobble with the LFO on amplitude (tremolo) — fits under a Reese / dubstep bass.", registry: subWobbleTools() });
  m.addModule({ id:"vocoder", label:"Vocoder / Talkbox", icon:"🤖", description:"Synthetic vocoder words — saw carrier shaped by vowel formant filters gated per step.", registry: vocoderTools() });
  m.addModule({ id:"noisefx", label:"Noise FX", icon:"💨", description:"Short pure-noise transitions (sweep up, sweep down, risetail, whoosh) for fills and accents.", registry: noiseFxTools() });
  m.addModule({ id:"cymbal", label:"Cymbal", icon:"🪘", description:"Crash / ride / china — metallic partials + bright noise + long decay.", registry: cymbalTools() });
  m.addModule({ id:"guitar", label:"Distorted Guitar", icon:"🎸", description:"Karplus-Strong string (or power chord) through hard-clip distortion and a resonant filter, with optional palm mute.", registry: guitarTools() });
  m.addModule({ id:"sitar", label:"Sitar", icon:"🪘", description:"Karplus-Strong string with a nonlinear bridge buzz and sympathetic-string shimmer.", registry: sitarTools() });
  m.addModule({ id:"steeldrum", label:"Steel Drum", icon:"🛢️", description:"Steel-pan tone (sine + steel-pan-characteristic inharmonic partials + soft mallet click).", registry: steelDrumTools() });
  m.addModule({ id:"accordion", label:"Accordion", icon:"🪗", description:"Two detuned reed oscillators per note (musette) with clipped-sine reed timbre and a slow bellows envelope.", registry: accordionTools() });
  m.addModule({ id:"theremin", label:"Theremin", icon:"👻", description:"Pure sine with continuous portamento glide between two notes, wide vibrato, no attack transient.", registry: thereminTools() });
  m.addModule({ id:"hihat808", label:"808 Hi-Hat", icon:"🎩", description:"Classic 808-style metallic hi-hat (six square-wave partials, high-passed), closed or open.", registry: hiHat808Tools() });
  m.addModule({ id:"stabhit", label:"Stab Brass Hit", icon:"💢", description:"A very short, hard-hitting brass/orchestra stab (saw stack + fast filter snap + noise transient).", registry: stabHitTools() });
