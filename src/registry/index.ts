...
export function createMasterRegistry(): MasterRegistry {
  ...
  m.addModule({ id:"sidechain",   label:"Sidechain", icon:"🔗", description:"Sidechain routing, issue detection and configuration.", registry: sidechainTools() });
  m.addModule({ id:"stereo",      label:"Stereo & Imaging", icon:"🎧", description:"Stereo field analysis, width, mid/side and auto-pan.", registry: stereoTools() });
  m.addModule({ id:"sampler",     label:"Sampler & Slicing", icon:"✂️", description:"Analyze, slice audio to MIDI and tweak slice parameters.", registry: samplerTools() });
  ...
  m.addModule({ id:"harmonic",    label:"DJ & Harmonic Mixing", icon:"🎶", description:"Camelot wheel, compatible tracks and transitions.", registry: harmonicTools() });
  ...
  m.addModule({ id:"freqsplit",   label:"Frequency Splitter", icon:"🎚️", description:"Split into bands, per-band processing, gain and solo.", registry: freqSplitTools() });
