...
window.LiveStudioPanels.projectsnapshot = function (panel, helpers) {
  ...
  async function refresh() {
    ...
    items.forEach((s) => {
      ...
      row.style.cssText = `display:grid;grid-template-columns:18px 1fr 150px 70px 70px 28px;gap:10px;align-items:center;padding:7px 9px;border:1px solid ${on ? "#6cc6ff" : "#2f2f36"};border-radius:8px;margin-bottom:6px;cursor:pointer`;
      row.innerHTML = `
        ...
        <button class="btn ghost ps-vscur" style="padding:3px 6px;font-size:11px" title="diff vs current unsaved state">vs now</button>
      ...
      row.onclick = (e) => {
        if (e.target.closest(".ps-restore") || e.target.closest(".ps-del") || e.target.closest(".ps-vscur")) return;
        const i = sel.indexOf(s.id);
        if (i >= 0) sel.splice(i, 1); else { sel.push(s.id); if (sel.length > 2) sel.shift(); }
        refresh(); if (sel.length === 2) doDiff();
      };
      row.querySelector(".ps-vscur").onclick = () => doDiffCurrent(s.id, s.label || "snapshot");
      row.querySelector(".ps-restore").onclick = async () => {
        const rr = await snap({ action: "restore", id: s.id });
        panel.querySelector("#ps-info").textContent = rr.success ? `Restored: ${rr.data.tracksRestored} tracks, ${rr.data.mixersRestored} mixers, ${rr.data.clipsRestored} clips, ${rr.data.scenesRestored} scenes, ${rr.data.markersRestored} markers (undoable via Edit History)` : rr.error;
      };
  ...
  function renderDiff(r, headerLabel) {
    ...
    box.innerHTML = `
      <div style="font-size:12px;color:#9a9aa2;margin-bottom:6px">${headerLabel}: <span style="color:#5ad17a">+${c.added}</span> <span style="color:#e24b4a">−${c.removed}</span> <span style="color:#ffb347">~${c.changed}</span></div>
  ...
  async function doDiff() {
    renderDiff(await snap({ action: "diff", idA: sel[0], idB: sel[1] }), "Diff (oldest → newest)");
  }
  async function doDiffCurrent(id, label) {
    renderDiff(await snap({ action: "diff_current", id }), `"${label}" vs current (unsaved) state`);
  }
  ...
  panel.doDiffCurrent = doDiffCurrent;
  refresh();
};
