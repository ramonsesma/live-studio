// Panel rico: DJ & Mezcla Armónica — rueda Camelot interactiva.
window.LiveStudioPanels = window.LiveStudioPanels || {};
window.LiveStudioPanels.harmonic = function (panel, helpers) {
  const exec = helpers.execute;
  panel.innerHTML = `
    <div class="panel-head"><h1>🎶 DJ & Mezcla Armónica</h1><p>Rueda Camelot: detecta la tonalidad de una pista y resalta sus compatibles.</p></div>
    <div class="ss-toolbar"><label class="hint">Pista</label><input id="hm-track" type="number" value="0" style="width:80px" /><button class="btn" id="hm-detect">Detectar tonalidad</button><span class="hint" id="hm-info"></span></div>
    <div class="hm-layout">
      <div id="hm-wheel" class="hm-wheel"></div>
      <div class="hm-side"><h3>Pistas compatibles</h3><div id="hm-compat" class="org-list-box"><span class="hint">—</span></div></div>
    </div>`;
  ...
  panel.querySelector("#hm-detect").onclick = async () => {
    ...
    if (comp.success) {
      for (const t of comp.data.compatibleTracks) {
        ...
        li.innerHTML = `<span>${t.name}</span><span class="hint">${t.camelotCode} · energía ${t.energyChange > 0 ? '+' : ''}${t.energyChange.toFixed(2)}</span>`;
        box.appendChild(li);
      }
    }
  };
